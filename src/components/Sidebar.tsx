...
import { BookOpen, Headphones, Settings, User, LogOut, ListMusic, ClipboardCheck, GraduationCap } from 'lucide-react';
import { useAuth } from '../lib/auth';
import { useTheme } from '../lib/theme';

export type Page = 'audio' | 'listening' | 'listeningTest' | 'vocabulary' | 'reading' | 'progress' | 'settings';
...
export function Sidebar({ onClose, currentPage, onNavigate }: SidebarProps) {
    ...
    return (
      <button
        ...
        className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition-colors text-left ${
          isActive
            ? `${theme.activeItem} ${theme.activeText}`
            : `${theme.hoverItem} text-slate-300`
        }`}
      >
        {icon}
        {label}
  ...
  return (
    <aside className={`w-64 ${theme.sidebar} text-slate-300 h-full flex flex-col shadow-xl md:shadow-none`}>
      ...
      <nav className="flex-1 px-4 py-6 md:py-0 space-y-2 overflow-y-auto">
        ...
        {navItem('vocabulary', 'Vocabulary', <GraduationCap size={20} />)}
